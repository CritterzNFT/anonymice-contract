...
*/
 ...
 |_|  |_|_|_||_\__|_|_||_\__, | |_| \_,_|_||_\__|\__|_\___/_||_/__/
  ...
  function currentCheethCost() public view returns (uint256) {
    ...
    if (_totalSupply > 2000 && _totalSupply <= 4000) return 1000000000000000000;
    if (_totalSupply > 4000 && _totalSupply <= 6000) return 2000000000000000000;
    if (_totalSupply > 6000 && _totalSupply <= 8000) return 3000000000000000000;
...
*/
  ...
  function hashToSVG(string memory _hash) public view returns (string memory) {
    ...
    for (uint8 i = 0; i < 9; i++) {
      ...
      for (uint16 j = 0; j < traitTypes[i][thisTraitIndex].pixelCount; j++) {
        ...
        uint8 x = letterToNumber(AnonymiceLibrary.substring(thisPixel, 0, 1));
        uint8 y = letterToNumber(AnonymiceLibrary.substring(thisPixel, 1, 2));
            ...
            bytes(
              string(
                abi.encodePacked(
                  ...
                  AnonymiceLibrary.encode(bytes(hashToSVG(tokenHash))),
                  '","attributes":',
    ...
    returns (string memory)
  {
    ...
    if (ownerOf(_tokenId) == 0x000000000000000000000000000000000000dEaD) {
      tokenHash = string(
        abi.encodePacked("1", AnonymiceLibrary.substring(tokenHash, 1, 9))
      );
    }
